...
class EventsResource(SyncAPIResource):
    ...
    def create(
        ...
        *,
        ...
        return self._post(
            path_template(
                "/apis/intake/v2/workspaces/{workspace}/entries/{name}/events", workspace=workspace, name=name
            ),
    ...
    def delete(
        ...
        *,
        ...
        if not workspace:
            raise ValueError(f"Expected a non-empty value for `workspace` but received {workspace!r}")
        if not entry:
            raise ValueError(f"Expected a non-empty value for `entry` but received {entry!r}")
        if not name:
            raise ValueError(f"Expected a non-empty value for `name` but received {name!r}")
        return self._delete(
            path_template(
