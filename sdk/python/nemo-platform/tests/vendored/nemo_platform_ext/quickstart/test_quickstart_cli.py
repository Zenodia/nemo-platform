...
from nemo_platform.cli.commands.quickstart.cli import (
...
from nemo_platform.quickstart.preflight import CheckStatus, PreflightResult
...
class TestQuickstartConfigureAutoTokenValidation:
    ...
    def test_auto_validates_ngc_api_key_from_environment(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with patch(
            "nemo_platform.quickstart.validate_ngc_credentials",
...
class TestQuickstartRegistryAuthCommand:
    ...
    def test_auth_validates_registry_token_from_options(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with (
            ...
            patch("nemo_platform.quickstart.prompts.prompt_password", return_value="test-token"),
        ):
...
class TestQuickstartUpCredentialPromptOnPullFailure:
    ...
    def test_up_prompts_for_credentials_when_pull_fails_without_image_flag(
        ...
        with (
            patch(
                "nemo_platform.quickstart.container.ContainerManager.pull_image_with_progress",
                fake_pull_with_progress,
            ),
            patch(
                "nemo_platform.quickstart.container.ContainerManager.is_image_available",
                return_value=False,
            ),
            patch(
                "nemo_platform.quickstart.prompts.prompt_for_registry_credentials",
                return_value=("nvcr.io", "$oauthtoken", SecretStr("test-key")),
            ) as mock_prompt,
            patch("nemo_platform.quickstart.is_interactive", return_value=True),
            patch("nemo_platform.quickstart.QuickstartCluster.preflight", return_value=[]),
            patch(
                "nemo_platform.quickstart.container.ContainerManager.start",
...
class TestQuickstartUpRequiresReservedGpuDeviceIdsWhenHostGpu:
    ...
    def test_up_does_not_fail_with_configure_message_when_reserved_gpu_device_ids_set(
        ...
        with patch("nemo_platform.quickstart.QuickstartCluster") as mock_cluster_class:
