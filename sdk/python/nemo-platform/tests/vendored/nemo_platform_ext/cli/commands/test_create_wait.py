...
from nemo_platform.cli.commands.api.inference.deployments import create_deployments
from nemo_platform.cli.commands.api.safe_synthesizer.jobs import create_jobs as create_safe_synthesizer_job
...
def test_safe_synthesizer_job_create_uses_input_name_for_wait_when_response_has_no_name() -> None:
    ...
    with (
        patch("nemo_platform.cli.commands.api.safe_synthesizer.jobs.handle_code_generation", return_value=False),
        patch("nemo_platform.cli.commands.api.safe_synthesizer.jobs.format_output"),
        patch(
            "nemo_platform.cli.commands.api.safe_synthesizer.jobs.wait_for_platform_job",
            return_value=True,
        ) as wait_for_platform_job,
    ):
...
def test_inference_deployment_create_exits_when_wait_fails() -> None:
    ...
    with (
        patch("nemo_platform.cli.commands.api.inference.deployments.handle_code_generation", return_value=False),
        patch("nemo_platform.cli.commands.api.inference.deployments.format_output"),
        patch(
            "nemo_platform.cli.commands.api.inference.deployments.wait_for_inference_deployment",
            return_value=False,
