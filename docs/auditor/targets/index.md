...
An `AuditTarget` identifies the model under test. It pairs a garak generator class (`type`) with a model identifier (`model`) and a generator-specific options dict (`options`). Targets are persisted in the {{platform_name}} entity store and referenced by name from `client.auditor.run(...)`.
...
Audit a NIM (or any OpenAI-compatible chat endpoint) routed through a {{platform_name}} provider:
