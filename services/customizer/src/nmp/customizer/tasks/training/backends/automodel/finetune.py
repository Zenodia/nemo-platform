...
"""
Customizer training script for Automodel fine-tuning.

This module wraps nemo_automodel's training recipes to add Customizer-specific
callbacks and logging using the Composition pattern. Supports SFT, Knowledge
Distillation, and Biencoder (embedding model) training.

Architecture:
    - AutomodelRecipe: Protocol defining the interface we need from Automodel recipes
    - CustomizerRecipeWrapper: Wraps any AutomodelRecipe and adds progress reporting
    - create_customizer_recipe(): Factory function that auto-detects and creates the appropriate recipe
"""
...
class CustomizerRecipeWrapper:
    """Wraps an Automodel recipe and adds Customizer-specific behavior.

    This wrapper uses composition to add progress reporting to any Automodel recipe,
    including SFT, Knowledge Distillation, and Biencoder (embedding) training.
    It monkey-patches the recipe's methods to intercept logging and checkpoint calls,
    forwarding them to the JobsServiceProgressReporter.
    """
    ...
    def __init__(self, recipe: AutomodelRecipe, job_ctx: NMPJobContext | None = None):
        ...
        logger.info(f"Customizer wrapper initialized: max_steps={self.max_steps}, num_epochs={self.num_epochs}")
    ...
    def run_train_validation_loop(self) -> None:
        """Run training with proper cleanup of Customizer resources."""
        try:
            ...
            if self.callback:
                self.callback.close()
                logger.info("Customizer callback closed")

    def _log_train_metrics(self, log_data: Any) -> None:
        """Wrapped log_train_metrics with Customizer reporting."""
        # Call original method first
        self._original_log_train_metrics(log_data)

        # Report to Customizer
    ...
    def _log_val_metrics(self, *args: Any, **kwargs: Any) -> None:
        """Wrapped log_val_metrics with Customizer reporting.
    ...
    def _save_checkpoint(
        ...
        """Wrapped save_checkpoint with Customizer reporting."""
        # Call original method first
...
def create_customizer_recipe(cfg: Any) -> CustomizerRecipeWrapper:
    """Factory function to create the appropriate wrapped recipe.

    Auto-detects the training type from the config:
    - Biencoder configs (model._target_ contains 'biencoder') -> biencoder recipe
    - KD configs (has teacher_model or kd_ratio) -> KD recipe
    - Otherwise -> SFT recipe

    Args:
        cfg: Configuration object from parse_args_and_load_config().

    Returns:
        A CustomizerRecipeWrapper wrapping the appropriate recipe for the training type.
    """
...
def main() -> None:
    """Main entry point for Customizer-enhanced training.

    Parses configuration, auto-detects the training type (biencoder, KD, or SFT),
    creates the appropriate recipe with Customizer integration, and runs training.
    """
    cfg = parse_args_and_load_config()
    recipe = create_customizer_recipe(cfg)
