...
# Observability for {{ngm_short_name}}

{{platform_name}} centrally manages OpenTelemetry across services. You can configure {{ngm_short_name}} to additionally enable tracing at the individual guardrail configuration level, providing visibility into how your rails execute - which rails fired, which actions ran, and how long each LLM call took.
...
2. **Guardrails execution spans** — Captured by the {{ngm_short_name}} instrumentation scope (`nemo_guardrails`). These are nested within the HTTP trace and cover the internal processing steps. For each interaction, a span is captured for each rail, which contains the internal action(s) and LLM call(s) made by the rail.
