...
from nemo_platform_ext.quickstart.prompts import (
...
def test_prompt_for_configuration_validates_new_ngc_key():
    ...
    with (
        patch("nemo_platform_ext.quickstart.prompts.prompt_password", return_value="  test-key\n"),
        patch("nemo_platform_ext.quickstart.prompts.prompt_choice", side_effect=["nvidia-build", "yes"]),
        patch(
...
def test_prompt_for_configuration_rejects_invalid_new_ngc_key():
    ...
    with (
        patch("nemo_platform_ext.quickstart.prompts.prompt_password", return_value="bad-key"),
        patch("nemo_platform_ext.quickstart.prompts.prompt_choice") as prompt_choice,
        patch(
            "nemo_platform_ext.quickstart.validators.validate_ngc_credentials",
...
def test_prompt_for_configuration_validates_existing_ngc_key_when_kept():
    ...
    with (
        patch("nemo_platform_ext.quickstart.prompts.prompt_password", return_value=""),
        patch("nemo_platform_ext.quickstart.prompts.prompt_choice", side_effect=["nvidia-build", "yes"]),
        patch(
            "nemo_platform_ext.quickstart.validators.validate_ngc_credentials",
            return_value=ValidationResult(True, "NGC credentials are valid"),
...
def test_prompt_for_configuration_surfaces_user_pass_registry_token_step():
    ...
    with (
        patch("nemo_platform_ext.quickstart.prompts.prompt_password", side_effect=["ngc-key", "registry-token"]),
        patch("nemo_platform_ext.quickstart.prompts.prompt_choice", side_effect=["yes", "nvidia-build", "yes"]),
        patch(
            "nemo_platform_ext.quickstart.prompts.prompt_text",
...
def test_prompt_for_configuration_surfaces_github_registry_token_step():
    ...
    with (
        patch("nemo_platform_ext.quickstart.prompts.prompt_password", side_effect=["ngc-key", "github-token"]),
        patch("nemo_platform_ext.quickstart.prompts.prompt_choice", side_effect=["yes", "nvidia-build", "yes"]),
        patch(
            "nemo_platform_ext.quickstart.prompts.prompt_text",
...
def test_prompt_for_configuration_can_skip_user_pass_registry_token_step():
    ...
    image = "ghcr.io/nvidia-nemo/nemo/nmp-api:latest"
    with (
        patch("nemo_platform_ext.quickstart.prompts.prompt_password", return_value="ngc-key"),
        patch("nemo_platform_ext.quickstart.prompts.prompt_choice", side_effect=["no", "nvidia-build", "yes"]),
        patch(
            "nemo_platform_ext.quickstart.validators.validate_ngc_credentials",
            return_value=ValidationResult(True, "NGC credentials are valid"),
        ),
...
def test_prompt_for_registry_credentials_returns_values():
    ...
    with (
        patch("nemo_platform_ext.quickstart.prompts.prompt_text", side_effect=["registry.example.com", "test-user"]),
        patch("nemo_platform_ext.quickstart.prompts.prompt_password", return_value="test-pass"),
        _valid_registry_credentials_patch() as validate_registry,
    ):
...
def test_prompt_for_registry_credentials_rejects_invalid_token():
    ...
    with (
        patch("nemo_platform_ext.quickstart.prompts.prompt_text", side_effect=["ghcr.io", "test-user"]),
...
def test_prompt_for_registry_credentials_uses_default_registry():
    ...
    with (
        patch(
            "nemo_platform_ext.quickstart.prompts.prompt_text", side_effect=["registry.example.com", "test-user"]
        ) as text_prompt,
        patch("nemo_platform_ext.quickstart.prompts.prompt_password", return_value="test-pass"),
...
def test_prompt_for_registry_credentials_two_part_image_no_namespace_as_registry():
    ...
    with (
        ...
        patch("nemo_platform_ext.quickstart.prompts.prompt_password", return_value="secret"),
        _valid_registry_credentials_patch(),
    ):
...
def test_prompt_for_registry_credentials_nvcr_uses_oauthtoken():
    ...
    with (
        patch("nemo_platform_ext.quickstart.prompts.prompt_text", return_value="nvcr.io") as text_prompt,
        patch("nemo_platform_ext.quickstart.prompts.prompt_password", return_value="my-ngc-token"),
        _valid_registry_credentials_patch() as validate_registry,
    ):
...
def test_prompt_for_registry_credentials_strips_password_whitespace():
    ...
    with (
        patch("nemo_platform_ext.quickstart.prompts.prompt_text", return_value="nvcr.io"),
        patch("nemo_platform_ext.quickstart.prompts.prompt_password", return_value="  my-ngc-token\n"),
        _valid_registry_credentials_patch() as validate_registry,
    ):
