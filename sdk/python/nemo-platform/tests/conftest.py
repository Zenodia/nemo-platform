...
from nemo_platform import NeMoPlatform, AsyncNeMoPlatform, DefaultAioHttpClient
from nemo_platform._utils import is_dict
...
logging.getLogger("nemo_platform").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[NeMoPlatform]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with NeMoPlatform(base_url=base_url, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncNeMoPlatform]:
