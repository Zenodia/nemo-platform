...
class TestArtifacts:
    ...
    def test_method_download(self, client: NeMoPlatform, respx_mock: MockRouter) -> None:
        respx_mock.get("/apis/evaluation/v2/workspaces/workspace/metric-jobs/job/results/artifacts/download").mock(
            return_value=httpx.Response(200, json={"foo": "bar"})
        )
        artifact = client.evaluation.metric_jobs.results.artifacts.download(
            job="job",
            workspace="workspace",
        )
    ...
    def test_raw_response_download(self, client: NeMoPlatform, respx_mock: MockRouter) -> None:
        ...
        artifact = client.evaluation.metric_jobs.results.artifacts.with_raw_response.download(
            job="job",
            workspace="workspace",
        )
    ...
    def test_streaming_response_download(self, client: NeMoPlatform, respx_mock: MockRouter) -> None:
        ...
        with client.evaluation.metric_jobs.results.artifacts.with_streaming_response.download(
            job="job",
            workspace="workspace",
        ) as artifact:
    ...
    def test_path_params_download(self, client: NeMoPlatform) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `workspace` but received ''"):
            client.evaluation.metric_jobs.results.artifacts.with_raw_response.download(
                job="job",
                workspace="",
            )
...
class TestAsyncArtifacts:
    ...
    async def test_method_download(self, async_client: AsyncNeMoPlatform, respx_mock: MockRouter) -> None:
        ...
        artifact = await async_client.evaluation.metric_jobs.results.artifacts.download(
            job="job",
            workspace="workspace",
        )
        assert artifact.is_closed
    ...
    async def test_raw_response_download(self, async_client: AsyncNeMoPlatform, respx_mock: MockRouter) -> None:
        ...
        artifact = await async_client.evaluation.metric_jobs.results.artifacts.with_raw_response.download(
            job="job",
            workspace="workspace",
        )
    ...
    async def test_streaming_response_download(self, async_client: AsyncNeMoPlatform, respx_mock: MockRouter) -> None:
        ...
        async with async_client.evaluation.metric_jobs.results.artifacts.with_streaming_response.download(
            job="job",
            workspace="workspace",
        ) as artifact:
            assert not artifact.is_closed
    ...
    async def test_path_params_download(self, async_client: AsyncNeMoPlatform) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `workspace` but received ''"):
            await async_client.evaluation.metric_jobs.results.artifacts.with_raw_response.download(
                job="job",
                workspace="",
            )
