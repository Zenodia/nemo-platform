...
from nemo_platform_ext.cli.commands.api.intake.apps import tasks
...
app = create_typer_app(name="apps", help="Manage apps")
...
def create_apps(
    ctx: typer.Context,
    name: Annotated[str | None, typer.Argument(help="App name (unique within workspace) (required)")] = None,
    workspace: Annotated[str | None, typer.Option("--workspace")] = None,
    description: Annotated[str | None, typer.Option("--description", help="App description")] = None,
    locked: Annotated[
        bool | None,
        typer.Option(
    ...
    project: Annotated[
        str | None, typer.Option("--project", help="The name of the project associated with this app")
    ] = None,
    exist_ok: Annotated[
        bool | None,
        typer.Option(
    ...
    [green]Examples:[/]
    nemo intake apps create <name> --input-file config.json
    nemo intake apps create <name> --input-data '{"name": "value"}'
    echo '{"json": "data"}' | nemo intake apps create <name> --input-file -
    nemo intake apps create <name> --<option> "value"
    """
    ...
    validate_required_fields(
        ...
        "intake apps create",
        {
            "name": "App name (unique within workspace) (required)",
        },
    )
    ...
    if handle_code_generation(["intake", "apps"], "create", all_kwargs, output_format, state):
        return

    client = state.get_client()
    result = client.intake.apps.create(**all_kwargs)
...
def delete_apps(
    ...
    client.intake.apps.delete(name, **kwargs)
...
def list_apps(
    ...
    if handle_code_generation(["intake", "apps"], "list", kwargs, output_format, state):
        return
    ...
    if all_pages:
        items = fetch_all_pages(
            client.intake.apps.list,
            path_args=path_args,
            body_args=kwargs,
            pagination_type=pagination_type,
        )
    else:
        items = client.intake.apps.list(*path_args, **kwargs)
...
def patch_apps(
    ...
    [green]Examples:[/]
    nemo intake apps patch <name> --input-file config.json
    nemo intake apps patch <name> --input-data '{"field": "value"}'
    echo '{"json": "data"}' | nemo intake apps patch <name> --input-file -
    nemo intake apps patch <name> --<option> "value"
    """
    # Read base input (optional if all fields provided via flags)
    if input_file or input_data:
        input_payload = read_data_input_with_flags(input_file=input_file, input_data=input_data)
    ...
    if handle_code_generation(["intake", "apps"], "patch", all_kwargs, output_format, state):
        return

    client = state.get_client()
    result = client.intake.apps.patch(**all_kwargs)
...
def retrieve_apps(
    ...
    if handle_code_generation(["intake", "apps"], "retrieve", kwargs, output_format, state):
        return

    client = state.get_client()
    result = client.intake.apps.retrieve(name, **kwargs)
