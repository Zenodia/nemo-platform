...
def import_task_main_without_heavy_runtime(monkeypatch):
    ...
    return importlib.import_module("nmp.safe_synthesizer.tasks.safe_synthesizer.__main__")
...
def test_upload_results_uploads_and_registers_adapter(tmp_path, monkeypatch):
    ...
    file_manager.upload.assert_has_calls(
        [
            call(adapter_path, "results/attempt-123/adapter"),
        ],
        any_order=True,
    )
