...
from nmp.customizer.entities.values import CheckpointFormat, FinetuningType, Precision, TrainingType
...
class DPOConfig(BaseModel):
    ...
    These control the DPO loss function and optimization behavior.
    """

    ref_policy_kl_penalty: float = Field(
        default=0.05,
        ge=0.0,
        description="KL penalty coefficient for reference policy divergence (beta in DPO paper)",
    )
    preference_average_log_probs: bool = Field(
        default=False,
        description="Whether to average log probabilities for preference loss calculation",
    )
    sft_average_log_probs: bool = Field(
        default=False,
        description="Whether to average log probabilities for SFT regularization loss",
    )
    preference_loss_weight: float = Field(
        default=1.0,
        ge=0.0,
        description="Weight for the preference (DPO) loss term",
    )
    sft_loss_weight: float = Field(
        default=0.0,
        ge=0.0,
        description="Weight for SFT regularization loss (0 = disabled)",
    )
...
class TrainingStepConfig(BaseModel):
    ...
    This is a normalized, backend-agnostic format that contains all
    the information needed to train. The TrainingBackend transforms
    this into library-specific configuration at runtime.
    """
