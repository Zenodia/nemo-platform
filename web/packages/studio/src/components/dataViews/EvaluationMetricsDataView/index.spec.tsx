...
const METRICS_URL = `${PLATFORM_BASE_URL}/apis/evaluation/v2/workspaces/:workspace/metrics`;

const emptyMetricsPage = {
  data: [],
  pagination: {
    page: 1,
    page_size: 50,
...
const captureRequests = () => {
  ...
  server.use(
    http.get(METRICS_URL, ({ request }) => {
      ...
      return HttpResponse.json(emptyMetricsPage);
    })
  );
  return requestUrls;
};
...
const renderDataView = (initialEntry: string) =>
  renderRoute(<EvaluationMetricsDataView workspace="default" />, {
    history: initialEntry,
    routes: [
      {
        path: '/workspaces/:workspace/evaluation/metrics',
        element: <EvaluationMetricsDataView workspace="default" />,
      },
    ],
  });

describe('EvaluationMetricsDataView', () => {
  afterEach(() => {
  ...
  it('sends no filter[ or search params on initial render', async () => {
    ...
    renderDataView('/workspaces/default/evaluation/metrics');
  ...
  it('sends filter[name][$like] when name search is active', async () => {
    const requestUrls = captureRequests();
    renderDataView('/workspaces/default/evaluation/metrics?s=foo');
  ...
  it('sends filter[created_at][$gte]/$lte when column filter is active', async () => {
    ...
    renderDataView(`/workspaces/default/evaluation/metrics?filters=${filters}`);
  ...
  it('never emits a top-level search or search[ key', async () => {
    ...
    renderDataView(`/workspaces/default/evaluation/metrics?s=foo&filters=${filters}`);
