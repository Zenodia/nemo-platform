...
from nemo_platform.quickstart.preflight import CheckStatus, PreflightChecker
from nemo_platform.quickstart.validators import ValidationResult, validate_image_registry_access
...
def test_validate_image_registry_access_inspects_manifest():
    ...
    with patch("nemo_platform.quickstart.validators.subprocess.run", return_value=completed) as run:
...
def test_validate_image_registry_access_reports_manifest_failure():
    ...
    with patch("nemo_platform.quickstart.validators.subprocess.run", return_value=completed):
        result = validate_image_registry_access("registry.example.com/org/image:tag")
...
def test_preflight_validates_user_pass_registry_access(user_pass_config, valid_user_pass_registry_credentials):
    ...
    with (
        patch(
            "nemo_platform.quickstart.validators.validate_image_registry_access",
...
def test_preflight_fails_when_stored_user_pass_registry_credentials_fail_validation():
    ...
    with (
        ...
        patch("nemo_platform.quickstart.validators.validate_image_registry_access") as validate_access,
    ):
        result = _registry_credentials_result(config)
...
def test_preflight_fails_when_user_pass_registry_credentials_are_not_configured():
    ...
    with (
        patch("nemo_platform.quickstart.validators.validate_registry_credentials") as validate_credentials,
        patch("nemo_platform.quickstart.validators.validate_image_registry_access") as validate_access,
    ):
        result = _registry_credentials_result(config)
...
def test_preflight_skips_manifest_check_for_registry_without_auth_requirement():
    ...
    with patch("nemo_platform.quickstart.validators.validate_image_registry_access") as validate_access:
        result = _registry_credentials_result(config)
