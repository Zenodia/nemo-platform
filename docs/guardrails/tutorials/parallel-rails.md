...
Run input and output rails in parallel to improve the response time of guardrail checks. This tutorial shows how to enable parallel rails using the {{platform_name}} Python SDK.
...
!!! note "Input rail mutations can lead to erroneous results during parallel execution because of race conditions that arise from the execution order and timing of parallel operations. This can result in output divergence compared to sequential execution. For such cases, use sequential mode."
