...
from nmp.customizer.entities.values import FinetuningType, TrainingType
...
class DatasetValidator:
    ...
    Example usage from dpo_config.py after prepare_dataset():
        ```python
        from nmp.customizer.tasks.training.datasets.preparation import prepare_dataset
        from nmp.customizer.tasks.training.datasets.validation import DatasetValidator
...
def detect_dpo_schema_name(file_path: str | Path) -> str:
    ...
    Returns:
        The NeMo RL dataset class name (e.g., "BinaryPreferenceDataset", "HelpSteer3Dataset")
    ...
    Example:
        ```python
        from nmp.customizer.tasks.training.datasets.preparation import prepare_dataset
        from nmp.customizer.tasks.training.datasets.validation import detect_dpo_schema_name
        ...
        # Detect the schema for NeMo RL configuration
        dataset_name = detect_dpo_schema_name(prepared.train_file)
        # dataset_name could be "BinaryPreferenceDataset", "HelpSteer3Dataset", etc.
        ```
    """
    ...
    if file_path.stat().st_size == 0:
        raise DatasetFormatError(f"Dataset file is empty: {file_path}")
    ...
    try:
        ...
        raise DatasetFormatError(f"First line of {file_path} is not valid JSON: {e}")
