...
"""Tests for benchmark job result routes configuration."""

from fastapi import FastAPI
from fastapi.openapi.utils import get_openapi
from nmp.common.api.utils import tweak_spec
from nmp.evaluator.api.v2.benchmarks.endpoints import _benchmark_jobs_router


class TestBenchmarkJobResultRoutes:
    """Tests verifying benchmark job result routes are correctly configured."""

    def test_benchmark_jobs_router_has_typed_result_routes(self):
        """Test that benchmark jobs router includes typed routes for aggregate-scores and row-scores."""
        app = FastAPI()
        # Use the rebased router which has /jobs prefix removed from paths
        app.include_router(_benchmark_jobs_router, prefix="/v2/workspaces/{workspace}/evaluation/benchmark-jobs")
        ...
        assert (
            "/v2/workspaces/{workspace}/evaluation/benchmark-jobs/{job}/results/aggregate-scores/download"
            in route_paths
        )
        assert "/v2/workspaces/{workspace}/evaluation/benchmark-jobs/{job}/results/row-scores/download" in route_paths

        # Verify the fallback wildcard route also exists
        assert "/v2/workspaces/{workspace}/evaluation/benchmark-jobs/{job}/results/{name}/download" in route_paths

    def test_benchmark_aggregate_scores_route_returns_json(self):
        """Test that aggregate-scores download route is configured for JSON response."""
        app = FastAPI()
        app.include_router(_benchmark_jobs_router, prefix="/benchmark-jobs")
        ...
        agg_scores_path = "/benchmark-jobs/{job}/results/aggregate-scores/download"
    ...
    def test_benchmark_row_scores_route_returns_jsonl(self):
        ...
        row_scores_path = "/benchmark-jobs/{job}/results/row-scores/download"
        ...
        assert content["application/jsonl"]["schema"]["$ref"] == "#/components/schemas/RowScore", content[
            "application/jsonl"
        ]["schema"]

    def test_benchmark_row_scores_route_has_limit_parameter(self):
    ...
    def test_benchmark_row_score_schema_excludes_error(self):
