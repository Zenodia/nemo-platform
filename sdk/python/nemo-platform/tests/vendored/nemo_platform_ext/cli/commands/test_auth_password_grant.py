...
def test_login_password_grant_with_flags(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("nemo_platform.cli.commands.auth.discover_nmp_config", lambda *_: _mock_oidc_config())
    monkeypatch.setattr(
        "nemo_platform.auth.device_flow.authenticate_with_password_grant",
        lambda **_: SimpleNamespace(token_for_nmp="access-token", refresh_token="refresh-token"),
    )
    monkeypatch.setattr(
        "nemo_platform.cli.commands.auth.decode_jwt_claims",
