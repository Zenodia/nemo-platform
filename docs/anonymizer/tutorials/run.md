...
This tutorial walks through the `anonymizer.run` job: defining a run spec, executing it locally or on the {{platform_name}} Jobs worker, and loading the parquet artifacts it produces.
...
Choose one execution path. Option A runs in the local CLI process. Option B submits the same request to the {{platform_name}} Jobs worker.
...
To execute the same spec on the {{platform_name}} Jobs worker instead of in the CLI process, use `run submit`:
