...
from ....types.inference.deployments.version_list_response import VersionListResponse
...
class VersionsResource(SyncAPIResource):
    ...
    def retrieve(
        ...
        *,
        ...
        if not deployment:
            raise ValueError(f"Expected a non-empty value for `deployment` but received {deployment!r}")
        if not name:
            raise ValueError(f"Expected a non-empty value for `name` but received {name!r}")
        return self._get(
            path_template(
                "/apis/models/v2/workspaces/{workspace}/deployments/{deployment}/versions/{name}",
                workspace=workspace,
    ...
    def list(
        ...
        *,
        ...
        return self._get(
            path_template(
                "/apis/models/v2/workspaces/{workspace}/deployments/{name}/versions", workspace=workspace, name=name
            ),
            options=make_request_options(
                extra_headers=extra_headers, extra_query=extra_query, extra_body=extra_body, timeout=timeout
            ),
