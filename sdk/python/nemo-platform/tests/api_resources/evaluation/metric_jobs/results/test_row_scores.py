...
class TestRowScores:
    ...
    def test_method_download(self, client: NeMoPlatform) -> None:
        row_score_stream = client.evaluation.metric_jobs.results.row_scores.download(
            job="job",
            workspace="workspace",
        )
    ...
    def test_raw_response_download(self, client: NeMoPlatform) -> None:
        response = client.evaluation.metric_jobs.results.row_scores.with_raw_response.download(
            job="job",
            workspace="workspace",
        )
    ...
    def test_streaming_response_download(self, client: NeMoPlatform) -> None:
        with client.evaluation.metric_jobs.results.row_scores.with_streaming_response.download(
            job="job",
            workspace="workspace",
        ) as response:
    ...
    def test_path_params_download(self, client: NeMoPlatform) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `workspace` but received ''"):
            client.evaluation.metric_jobs.results.row_scores.with_raw_response.download(
                job="job",
                workspace="",
            )
...
class TestAsyncRowScores:
    ...
    async def test_method_download(self, async_client: AsyncNeMoPlatform) -> None:
        row_score_stream = await async_client.evaluation.metric_jobs.results.row_scores.download(
            job="job",
            workspace="workspace",
        )
    ...
    async def test_raw_response_download(self, async_client: AsyncNeMoPlatform) -> None:
        response = await async_client.evaluation.metric_jobs.results.row_scores.with_raw_response.download(
            job="job",
            workspace="workspace",
        )
    ...
    async def test_streaming_response_download(self, async_client: AsyncNeMoPlatform) -> None:
        async with async_client.evaluation.metric_jobs.results.row_scores.with_streaming_response.download(
            job="job",
            workspace="workspace",
        ) as response:
            assert not response.is_closed
    ...
    async def test_path_params_download(self, async_client: AsyncNeMoPlatform) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `workspace` but received ''"):
            await async_client.evaluation.metric_jobs.results.row_scores.with_raw_response.download(
                job="job",
                workspace="",
            )
