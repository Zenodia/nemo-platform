...
``startup`` / ``shutdown`` hooks on :class:`RequestProcessor`. On each
inbound request it builds a normalized
:class:`RoutingRequestSummary`, asks the plugin for a tier label, and
stamps the decision on ``ctx.metadata`` so the downstream
:class:`OSSRouterLLMBackend` can dispatch.
...
from switchyard.lib.backends.oss_router_llm_backend import CTX_OSS_ROUTER_TIER
...
class PluginRoutingRequestProcessor(RequestProcessor):
    ...
    def _apply_decision(
        ...
        request.body["model"] = self._tier_models[decision.tier]  # type: ignore[attr-defined]
        return request
    ...
    def _handle_route_error(
        ...
        request.body["model"] = self._tier_models[chosen]  # type: ignore[attr-defined]
        return request
    ...
    def _handle_failure(
        ...
        *,
        ...
        request.body["model"] = self._tier_models[self._fallback_tier]  # type: ignore[attr-defined]
        return request
