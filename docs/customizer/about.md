...
This page provides an overview of the customization concepts for the {{platform_name}}.
...
{{ncm_short_name}} supports supervised fine-tuning (SFT) with the PEFT method of Low-Rank Adaptation (LoRA).
...
Use {{ncm_short_name}} to train custom models on your own data. The workflow can be carried out as follows:
- Upload a dataset
...
{{platform_name}} Customizer supports various distributed training parallelization methods, which can be mixed together.
