...
from nmp.customizer.tasks.training.utils import generate_torchrun_flags_from_env
...
class AutomodelBackend(TrainingBackend):
    """
    Implements the TrainingBackend protocol for nemo-automodel library.
    """
    ...
    def backend_type(self) -> TrainingBackendEnum:
        return TrainingBackendEnum.AUTOMODEL
    ...
    def execute_training(
        ...
        command.extend(
            [
                "-m",
                "nmp.customizer.tasks.training.backends.automodel.finetune",
                "--config",
